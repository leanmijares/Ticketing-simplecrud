...
if(!isset($_SESSION['user_type'])){
 ...
 if(isset($_POST['requestlevel'])){
 
        $ticket_request_type = trim($_POST['ticket_request_type']);
        $isValid = true;
        
        
        if($isValid) {
        $insertSQL="INSERT INTO ticket_request set ticket_request_type=' $ticket_request_type'";
            ...
            <div class="card">
              <form class="" method="post">
                <div class="card-body m-2">
                    <h6 class="card-subtitle mb-2 text-muted">Add Type of Request</h6>
                <p class="card-text">
                    <div class="form-group row">
                        <label class="mb-2 text-muted">Label Display Name</label>
                        <div class="form-group w-100 mb-0">
                            <input type="text" class="form-control" name="ticket_request_type" required>
                        </div>
                    </div>
                </p>
                <?php 
              ...
              <div class="card-footer"><button type="submit" name="requestlevel" class="btn" style="background-color: #44649c; font-size: 13px;">Add New Level</button></div>
              </form>
            </div>
         </div>  
       </div>
    ...
    <div class="container-fluid">
            <div class="card shadow mb-4">
                <div class="card-header py-3">
                  <h6 class="m-0 text-muted">Request Lists</h6>
                </div>
                <div class="card-body m-2">
                ...
                <tbody>
                     ...
                     <?php
                     $query = $conn->query("SELECT * FROM ticket_request");
                     while($result = $query->fetch_array()) { ?> 
                    <tr>
                        <td><?php echo $result['ticket_request_id'] ?></td>
                        <td class="hidetext"><?php echo $result['ticket_request_type'] ?></td>
                        <td> <div class="text-center">
                                <a href="admin_type_request_delete.php?ticket_request_id=<?php echo $result["ticket_request_id"]; ?>" class="btn btn-danger btn-circle" data-toggle="tooltip"
